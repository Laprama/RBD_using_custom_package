...
#This script is edited to run mean and std experiments on pli connectivity data
...
X = pd.read_hdf(folder + 'test/' + data_type + '_' + method + '_mean_and_std_' + '_df.h5' )
...
subsets_dict['all_features'] = list(X.drop(columns = [col for col in X.columns if 'all' in col]).columns)

save_folder = '/user/home/ko20929/work/RBD_using_custom_package/Execute New Experiments/Connectivity/Connectivity_Results/Experiment_set_3/'


for expt_num in [1,2,3,4]:
    clfs_dict =  {'RF' : RandomForestClassifier(), 'DT' : DecisionTreeClassifier() , 'Ada_B' : AdaBoostClassifier() , 'SVM' : SVC()} 
    save_path = save_folder + data_type + method + '_expt_' + str(expt_num) + '_mean_std_'
