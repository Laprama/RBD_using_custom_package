...
folder = 'tuned_models_and_results/dual_region_' + str(r_state)
model = list(clf_dict.keys())[0]
save_name = model + '_' + data_type + '_' + TS_Fresh_setting + '_' + connectivity_setting + '_' + 'expt' + str(expt_num) + '_' +  str(num_splits) + '_fold_results_df.pkl'
...
if os.path.exists(save_path):
    ...
    run_expt = False
    run_expt = True #This allows me to deliberately overfit the Random Forest
else:
...
results_df, clfs_dict = run_expts.run_mv_tsc(X_expt,y_expt,groups_expt,clf_dict, return_df = True , subset_names_and_cols = combined_regions_features_dict, random_states = [int(r_state)], groups_for_fit = True, best_params = True, return_clfs = True)

#Save the results dataframe 
#Commenting this out for now for overfitting the RF 
folder = 'tuned_models_and_results/dual_region_rstate' + str(r_state)
model = list(clf_dict.keys())[0]
save_name = model + '_' + data_type + '_' + TS_Fresh_setting + '_' + connectivity_setting + '_' + 'expt' + str(expt_num) + '_' +  str(num_splits) + '_fold_results_df.pkl'
joblib.dump(results_df , folder + save_name)

clfs_save_name = model + '_' + data_type + '_' + TS_Fresh_setting + '_' + connectivity_setting + '_' + 'expt' + str(expt_num) + '_' +  str(num_splits) + '_clfs_dict.pkl'
